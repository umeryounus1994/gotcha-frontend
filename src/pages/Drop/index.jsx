...
const Drop = () => {
  ...
  const handleAddDrop = useMutation(
    ...
    {
      onMutate: () => {
        setLoading(true); // Show GIF when mutation starts
      },
      onSuccess: (response) => {
        setTimeout(() => {
          setLoading(false); // Hide GIF on success
        }, 7000);
      ...
      onError: (err) => {
        setLoading(false); // Hide GIF on error
    ...
    <Box bg="white" style={{ borderRadius: "5px" }}>
      ...
      <Flex gap="lg" p="md" wrap={{ base: "wrap", lg: "nowrap" }}>
        ...
        <div style={{
          ...
          <img src="https://icons-gotcha.nyc3.digitaloceanspaces.com/dropoffer.gif" alt="Loading..." />
        </div>
      )}
          <LoadScript
            id="script-loader"
            ...
            <Box style={{ minHeight: "600px" }}>
              ...
              >
                {markers.map((obj, ind) => (
                  <Marker
                  icon={{
                    url: 'https://icons-gotcha.nyc3.digitaloceanspaces.com/WhatsApp_Image_2024-08-27_at_15.16.08-removebg-preview.png',
                    ...
                    onDragEnd={(e) => {
                      ...
                      const newLocations = newMarkers.map(marker => ({ lat: marker.lat, lng: marker.lng }));
